...
Sheet = (function() {
  ...
  Sheet.prototype.set = function(col, row, str) {
    if ((str != null) && str !== '') {
      //check if this is truly a string, or if there is a decimal, treat as currency
      var numericValue = +str;
      if(isNaN(numericValue) || (!isNaN(numericValue) && str !== null && str.toString().indexOf('.') === -1)) {
        return this.data[row][col].v = this.book.ss.str2id('' + str);
      } else {
        this.data[row][col].v = str;
        this.data[row][col].s = (this.book.st.numstyle2id({type:"currency"}) -1);
...
Style = (function() {
  ...
  Style.prototype.numformat2id = function(numformat) {
    var id, k;
    numformat || (numformat = {});
    numformat.format_code || (numformat.format_code = "$ #,##0.00;$ #,##0.00");
    numformat.type || (numformat.type = "currency");
    numformat.format_id || (numformat.format_id = "164");
    k = 'numformat_' + numformat.type;
    id = this.cache[k];
    if (id) {
      return id;
    } else {
      this.numformats.push(numformat);
      this.cache[k] = this.numformats.length;
      return this.numformats.length;
    }
  };
  
  Style.prototype.numstyle2id = function(numformat) {
    return this.mstyle.length + this.numformat2id(numformat);
  };


  Style.prototype.toxml = function() {
    var bders, cs, e, ea, es, fills, numFormats, fonts, o, ss, _i, _j, _k, _l, _n, _len, _len2, _len3, _len4, _ref, _ref2, _ref3, _ref4, _ref5;
    ...
    numFormats = ss.ele('numFmts', {
        count: this.numformats.length
    });
    _ref5 = this.numformats;
    for (_n = 0, _len = _ref5.length; _n < _len; _n++) {
      o = _ref5[_n];
      e = numFormats.ele('numFmt', {numFmtId: o.format_id, formatCode: o.format_code});
    }
    fonts = ss.ele('fonts', {
    ...
    cs = ss.ele('cellXfs', {
      count: (this.mstyle.length + this.numformats.length)
    });
    _ref4 = this.mstyle;
    ...
    _ref4 = this.numformats;
     for (_n = 0, _len = _ref4.length; _n < _len; _n++) {
      o = _ref4[_n];
      e = cs.ele('xf', {
        numFmtId: o.format_id,
        fontId: '0',
        fillId: '0',
        borderId: '0',
        xfId: '0',
